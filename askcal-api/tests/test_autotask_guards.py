...
def test_a_confident_classification_still_tasks():
    assert should_auto_task(signals(), track_row(), 70)
...
def test_a_guess_does_not_become_a_task():
    ...
    """
    assert not should_auto_task(signals(confidence=0.2), track_row(), 70)


def test_the_confidence_floor_is_configurable(monkeypatch):
    monkeypatch.setenv("ASKCAL_AUTO_TASK_MIN_CONFIDENCE", "0.95")
    get_settings.cache_clear()
    assert not should_auto_task(signals(confidence=0.9), track_row(), 70)
...
def test_no_action_required_never_tasks():
    assert not should_auto_task(
        signals(action_required=False), track_row(), 70
    )
...
def test_zero_stakes_never_tasks(consequence):
    assert not should_auto_task(
        signals(consequence=consequence), track_row(), 70
    )


def test_feed_never_tasks():
    assert not should_auto_task(signals(track="feed"), track_row(auto_tasks=False), 70)


def test_an_inactive_track_never_tasks():
    assert not should_auto_task(signals(), track_row(active=False), 70)
...
def test_automated_senders_still_task():
    ...
    assert should_auto_task(
        signals(sender_type="automated_system"), track_row(), 70
    )
...
def test_raising_confidence_stops_a_borderline_mail_auto_tasking():
    ...
    track_row = SimpleNamespace(active=True, auto_tasks=True)

    assert should_auto_task(signals, track_row, 60, _User())
    assert not should_auto_task(
