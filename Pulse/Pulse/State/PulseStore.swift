...
//  starts empty and fills from pulse-api once an account is connected.
//
...
final class PulseStore {
    ...
    /// True once a Pulse account is connected — data comes from pulse-api.
    var isLive = false
    ...
    func tasks(in track: TrackKey) -> [PulseTask] {
        openTasks.filter { $0.track == track }
    }

    func task(id: UUID) -> PulseTask? {
        tasks.first { $0.id == id }
    }

    func slot(for task: PulseTask) -> PlanSlot? {
        dayPlan.first { $0.taskId == task.id }
    }

    // MARK: - Time-of-day grouping

    private func dayPart(of task: PulseTask) -> DayPart {
    ...
    var groupedSchedule: [(part: DayPart, tasks: [PulseTask])] {
    ...
    private func loadLocal() {
        ...
        if let d = ud.data(forKey: Self.localTasksKey),
           let t = try? JSONDecoder().decode([PulseTask].self, from: d) { tasks = t }
    ...
    func toggleDone(_ task: PulseTask) {
    ...
    func updateTaskSchedule(
        _ task: PulseTask, scheduledAt: Date?, dueAt: Date?, scheduledFor: Date?
    ) {
        Haptics.tick()
        if isLive {
            Task {
    ...
    func review(_ task: PulseTask, done: Bool) {
        guard let idx = tasks.firstIndex(where: { $0.id == task.id }) else { return }
