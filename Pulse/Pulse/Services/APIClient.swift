...
//  Pulse
//
//  pulse-api client. Access token lives in memory (15-min JWT), refresh
...
final class APIClient {
    ...
    var isConnected: Bool { Keychain.read("pulseRefreshToken") != nil }
    ...
    /// pulse://oauth#accessToken=…&refreshToken=…&email=…&name=…
    func handleAuthCallback(_ url: URL) -> (email: String, name: String)? {
        ...
        accessToken = access
        Keychain.save("pulseRefreshToken", refresh)
        return (params["email"] ?? "", params["name"] ?? "")
    }

    func disconnect() {
        accessToken = nil
        Keychain.delete("pulseRefreshToken")
    }

    private func refreshAccessToken() async throws {
        guard let refresh = Keychain.read("pulseRefreshToken") else {
    ...
    struct TasksOut: Decodable { let tasks: [PulseTask] }
    ...
    func tasks() async throws -> [PulseTask] {
    ...
    func setTaskStatus(_ id: UUID, _ status: TaskStatus) async throws -> PulseTask {
    ...
    func tasks(on date: Date) async throws -> [PulseTask] {
    ...
    func handleEmail(_ gmailId: String) async throws -> PulseTask {
